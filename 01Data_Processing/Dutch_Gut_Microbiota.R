...
#Read data
data_info <- read.table("./Dutch_Gut_Microbiota/PMID35115690_studies_export.tsv",header=T,sep="\t")


start = 1
end = 412

write.table(c(),paste0(workpath,"/Dutch_Gut_Microbiota/res_significant_snp_clump.txt",start,"_",end),row.names=F,col.names=F,quote=F,sep="\t")
...
for (i in start:end) {
  ...
  dat1 <- read_exposure_data(
    filename = paste0(workpath,"/Dutch_Gut_Microbiota/",file_name,"_buildGRCh37.tsv"),
  ...
  print(paste0("read rs_",file_name," ",i," ok 412"))
...
#----------------------------------Local Clump-----------------------------------
  ...
  write.table(res[,c("SNP","effect_allele.exposure","other_allele.exposure","beta.exposure","se.exposure","eaf.exposure","pval.exposure","dataid")],paste0(workpath,"/Dutch_Gut_Microbiota/res_significant_snp_clump.txt",start,"_",end),row.names=F,col.names=F,quote=F,sep="\t",append=T) 
}
...
snp_all_clump <- read.table("./Dutch_Gut_Microbiota/res_significant_snp_clump.txt",header=F)
...
#bmi t2d smoke123
snp_5_clump <- read.table("./Dutch_Gut_Microbiota/significant_clump_all_files.txt",header=T)
...
for (i in start:end) {
  ...
  dat1 <- read.table(paste0(workpath,"/Dutch_Gut_Microbiota/",file_name,"_buildGRCh37.tsv"),header=T,sep="\t")
  ...
  write.table(dat1_res,paste0(workpath,"/Dutch_Gut_Microbiota/Processed_data/",i,"_res_",file_name,".txt"),row.names=F,col.names=T,quote=F,sep="\t")
  R.utils::gzip(paste0(workpath,"/Dutch_Gut_Microbiota/Processed_data/",i,"_res_",file_name,".txt"))
}
